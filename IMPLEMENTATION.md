...
- [x] Table verification notebook (`jobs/verify_tables.py`, 2026-07-02) — 13 manual checks across three layers: nightly tables (verifiable after 2-3 runs), decision loop (2-3 trading days), exit lifecycle (event-driven, first closed position). Empty result = pass for the offending-rows checks
- [x] Market-open guard in `jobs/decision_loop.py` (2026-07-02) — pre-flight Alpaca clock check; on holidays (cron only knows weekdays) resolves today's PENDING rows to SKIP with reason 'market closed' instead of queuing orders that would fill at the next open
- [x] Trades table writer (2026-07-02) — decision_loop records each market order + GTC stop at submission via `order_to_trade_record` (`src/execution/alpaca.py`); filled_price/filled_at usually null at write time since market orders fill after the response returns
- [x] Idempotency audit of both workflows (2026-07-02) — nightly pipeline was already safe (date-named landing files, DLT silver dedup, merge/delete-then-append writers); `run_quant_signals` was a blind append and could stack duplicate PENDING rows (double-buy risk) — fixed: deletes today's PENDING before appending and skips tickers with an already-resolved decision today
